...
`round` is a minimal package that tweaks the behaviour of `base::round()` to be in line with rounding conventions. By default `base::round()` rounds numbers ending with 5 to even. For example: 
...
In contrast, `round::round()` will always round numbers ending with 5 up: 
...
At this stage, `round::round()` cannot take negative integers as the `digits` argument, unlike `base::round()` which uses negative integers to round to the power of ten: 
...
Use at your own risk. This is a toy package I've put together to learn more about building packages in R. 

If you do want to use it, you can install the development version of round from [GitHub](https://github.com/) with:
